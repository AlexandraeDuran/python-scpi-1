...
class cmd57_console(cmd57.cmd57):
    ...
    def print_sys_info(self):
        ...
        print("Installed options:    %s" %
              " ".join(self.ask_installed_options()))
    ...
    def print_man_config(self):
        ...
        print("  Expected power:   %s dBm" %
              format_float(self.ask_bts_expected_power()))
        print("  Used TS power:    %s dBm" %
              format_float(self.ask_bts_tch_tx_power()))
        print("  Mode:             %s" % self.ask_bts_tch_mode())
        print("  Timing advance:   %s qbits" %
              format_int(self.ask_bts_tch_timing()))
        print("  Input bandwidth:  %s" % self.ask_bts_tch_input_bandwidth())

    def print_man_bidl_info(self):
        print("Manual BTS test - Synchronized mode (no signaling)")
        print("  Peak power:         %s dBm" %
              format_float(self.ask_peak_power()))
    ...
    def print_man_bbch_info(self, update=False):
        ...
        (pk_phase_err_match, avg_phase_err_match,
         freq_err_match) = self.ask_phase_freq_match()
        print("Manual test - Control Channel")
        print("  RF channel:           %d" % self.ask_bts_ccch_arfcn())
        print("    Frequency error:    %s Hz  (%s)" %
              (format_int(freq_err), freq_err_match))
        print("    Phase Error (PK):   %s deg (%s)" %
              (format_float(self.fetch_phase_err_pk()), pk_phase_err_match))
        print("    Phase Error (AVG):  %s deg (%s)" %
              (format_float(self.fetch_phase_err_rms()), avg_phase_err_match))
        print("  BTS power:            %s dBm" %
              format_float(self.ask_peak_power()))
    ...
    def print_man_spectrum_modulation(self, update=False):
        ...
        print("  Absolute tolerance mask (dBm):          ",
              self.ask_spectrum_modulation_tolerance_abs())
        print("  Relative tolerance mask (dBc):          ",
              self.ask_spectrum_modulation_tolerance_rel())
        print("  Measurement offsets (kHz):              ",
              self.fetch_spectrum_modulation_offsets())
        if update:
        ...
        print("  Mask match?                             ",
              self.ask_spectrum_modulation_match())

    def print_man_spectrum_switching(self, update=False):
        print("Spectrum due to Switching:")
        print("  Absolute tolerance mask (dBm):          ",
              self.ask_spectrum_switching_tolerance_abs())
        print("  Relative tolerance mask (dBc):          ",
              self.ask_spectrum_switching_tolerance_rel())
        print("  Measurement offsets (kHz):              ",
              self.fetch_spectrum_switching_offsets())
        if update:
        ...
        print("  Measured values (dBc):                  ", spectrum)
        print("  Mask match?                             ",
              self.ask_spectrum_switching_match())
    ...
    def print_mod_info(self, update=False):
        ...
        print("  Peak power:         %s dBm" %
              format_float(self.ask_peak_power()))
        print("  Avg. burst power:   %s dBm" %
              format_float(self.fetch_burst_power_avg()))
        print("  Power ramp:         %s" % self.ask_power_mask_match())
        print("  Frequency error:    %s Hz  (%s)" %
              (format_int(self.fetch_freq_err()), freq_err_match))
        print("  Phase Error (PK):   %s deg (%s)" %
              (format_float(self.fetch_phase_err_pk()), pk_phase_err_match))
        print("  Phase Error (AVG):  %s deg (%s)" %
              (format_float(self.fetch_phase_err_rms()), avg_phase_err_match))
    ...
    def print_ber_test_settings(self):
        ...
        print("  Unused TS power:    %s dB" %
              ("OFF" if power_ts_unused is None else ("%.1f" % power_ts_unused),))
        ...
        print("  Class Ib         %6d     %6d" % (
            self.ask_ber_limit_class_1b(), self.ask_ber_max_class_1b_samples()))
        print("  Class II         %6d     %6d" %
              (self.ask_ber_limit_class_2(), self.ask_ber_max_class_2_samples()))
        print("  Erased Frames    %6d     %6d" % (
            self.ask_ber_limit_erased_frames(), self.ask_ber_max_erased_frames_samples()))
    ...
    def print_ber_test_result(self, update=False):
        ...
        if res in ["PASS", "FAIL"]:
            (ber1b_events, ber1b_ber, ber1b_rber) = (self.fetch_ber_class_1b_events(
            ), self.fetch_ber_class_1b_ber(), self.fetch_ber_class_1b_rber())
            (ber2_events, ber2_ber, ber2_rber) = (self.fetch_ber_class_2_events(
            ), self.fetch_ber_class_2_ber(), self.fetch_ber_class_2_rber())
            (fer_events, fer_percent) = (
                self.fetch_ber_erased_events(), self.fetch_ber_erased_fer())
            crc_errors = self.fetch_ber_crc_errors()
            print("                events    BER       RBER")
            print("  Class Ib      %6d  %7.3f%%  %7.3f%%" %
                  (ber1b_events, ber1b_ber, ber1b_rber))
            print("  Class II      %6d  %7.3f%%  %7.3f%%" %
                  (ber2_events, ber2_ber, ber2_rber))
            print("                events    FER")
