        ...
        try:
            while self.serial_alive:
                for method in self._current_states:
                    ...
                    if self._current_states[method] != self._previous_states[
                        method]:
                        print(" *** %s changed to %d *** " % (method,
                                                              self._current_states[
                                                                  method]))
                        self._previous_states[method] = self._current_states[
                            method]
                ...
                if (len(self.input_buffer) > 0 and
                    ...
                    # Got a message, parse it (sans the CRLF) and empty the
                    # buffer
